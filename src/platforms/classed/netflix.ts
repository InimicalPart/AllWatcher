...
export default class Netflix extends Platform {
    ...
    public async run() {
        ...
        await global.browser.evaluate(this.tab, `
            if (!(typeof skipIntroInterval != "undefined") && ${global?.config?.netflix?.autoSkipIntro ?? false}) {
                skipIntroInterval = setInterval(()=>{
                    Array.from(document.querySelectorAll("span")).filter((a) => a.innerText == "Skip intro").forEach((a) => a.click())
                },100)
        ...
        const episode_title = (this.cache[episodeID]?.episode_title ?? await global.browser.evaluate(this.tab, `(document.querySelector("[data-uia$='video-title'] span:nth-child(3)") ?? " ")?.textContent || undefined`, "episodeTitleGrab") as string ?? "N/A").replace("’","'")
        const episode = this.cache[episodeID]?.episode             || await global.browser.evaluate(this.tab, `parseInt(document.querySelector("[data-uia$='video-title'] span")?.textContent?.replace("E","")??"0")`, "episodeNumberGrab") as number
        const playing = await Netflix.isPlaying(this.tab)
        
        const season = await this.getSeason(title, episode, episode_title) ?? "?"
        ...
        const totalEpisodesInSeason = season ? this.seasonsCache[title].episodes.filter(e => e.season_number == season).length : "?"
        const totalSeasons = this.seasonsCache[title].episodes.reduce((acc, e) => acc.includes(e.season_number) ? acc : [...acc, e.season_number], []).length
